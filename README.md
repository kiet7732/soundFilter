...
-   **Karaoke Features:**
    -   AI-powered lyrics extraction and word-level synchronization
    -   Interactive lyrics editor with real-time timing recalculation
    -   Karaoke video rendering engine with custom background images
    -   MP4 video export with synchronized lyrics overlay

## Technologies Used

| Component    | Technology                                                                                            |
| :----------- | :---------------------------------------------------------------------------------------------------- |
| **Frontend** | React, TypeScript, Vite, Tailwind CSS, Lucide React, Axios, JSZip, File-saver, WaveSurfer.js         |
| **Backend**  | Python, FastAPI, Uvicorn, Demucs, AudioSep, PyTorch, Torchaudio, Whisper, FFmpeg                     |
| **GPU**      | CUDA                                                                                                  |
...
└── Frontend/                    # React Frontend 
    ├── src/
    │   ├── app/
    │   │   ├── types           # TypeScript type definitionsefinitions
...
5.  **Completion:** When the AI task finishes running, it saves the result files (MP3 format, 192kbps) to the `data/results/{task_id}/` directory and creates a `status.json` file to signal completion.
...
8.  **Karaoke (Music Mode):** Users can edit lyrics, upload background images, and render karaoke videos with synchronized lyrics.
